...
function ServiceCard({ service }: { service: (typeof SERVICES)[number] }) {
  ...
  return (
    <article className="flex h-full flex-col rounded-3xl border border-ink/10 bg-background/60 p-7 backdrop-blur md:min-h-[600px] md:p-8">
      <div className="md:min-h-[230px]">
        <div className="font-display text-5xl text-accent md:text-6xl">{service.number}</div>
        <h3 className="mt-4 font-display text-3xl leading-tight text-ink md:mt-5 md:min-h-[88px] md:text-4xl">
          {service.title}
        </h3>
        <p className="mt-4 text-base leading-relaxed text-ink-soft md:mt-5 md:min-h-[96px]">
          {service.blurb}
        </p>
      </div>

      <div className="mt-6 flex flex-wrap items-start gap-x-3 gap-y-1 md:mt-7 md:min-h-[80px]">
      ...
      <ul className="mt-6 grid gap-3 border-t border-ink/10 pt-6 md:mt-7 md:min-h-[220px] md:pt-7">
